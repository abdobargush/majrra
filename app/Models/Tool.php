...
class Tool extends Model
{
  ...
  public function setThumbnailAttribute($value)
  {
    ...
    $destination_path = "tools";

    // if the image was erased
    if ($value == null) {
      // delete the image from disk
      Storage::delete($this->getRawOriginal($attribute_name));
    ...
    if (Str::startsWith($value, 'data:image')) {
      ...
      Storage::put("{$destination_path}/{$filename}", $image->stream(), 'public');

      // 3. Delete the previous image, if there was one.
      Storage::delete("{$destination_path}/" . $this->getRawOriginal($attribute_name));
